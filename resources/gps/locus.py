...
# Based on <https://github.com/don/locus/blob/master/locus.py>
...
if __name__ == "__main__":
    ...
    path = sys.argv[1]
    coords = parseFile(path)

    print "%s coords parsed" % len(coords)

    valid_by_author = [c for c in coords if c.fix < 5]
    print "%s coords 'valid' by original author's metric" % len(valid_by_author)

    valid_by_year = [c for c in coords if c.datetime.year == 2022]
    print "%s coords for 2022" % len(valid_by_year)
